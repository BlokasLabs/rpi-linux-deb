...
printf "#!/bin/sh -e\n\n" | tee raspberrypi-kernel-rt.postinst | tee raspberrypi-bootloader.postinst | tee raspberrypi-kernel-rt.preinst > raspberrypi-bootloader.preinst

cat <<EOF | tee -a raspberrypi-kernel-rt.postinst >> raspberrypi-bootloader.postinst
get_file_list() {
  cat /var/lib/dpkg/info/raspberrypi-kernel-rt.md5sums /var/lib/dpkg/info/raspberrypi-kernel-rt.md5sums 2> /dev/null | awk '/ boot/ {print "/"\$2}'
}
...
for FN in ../boot/kernel.img ../boot/kernel7.img; do
  if [ -f "$FN" ]; then
    FN=${FN#../boot/}
    cat << EOF >> raspberrypi-kernel-rt.postinst
...
EOF
for FN in ../boot/kernel7l.img; do
  if [ -f "$FN" ]; then
    FN=${FN#../boot/}
    cat << EOF >> raspberrypi-kernel-rt.postinst
  if [ -f /usr/share/rpikernelhack/$FN ]; then
...
cat <<EOF >> raspberrypi-kernel-rt.preinst
...
printf "#DEBHELPER#\n" | tee -a raspberrypi-kernel-rt.postinst | tee -a raspberrypi-bootloader.postinst | tee -a raspberrypi-kernel-rt.preinst >> raspberrypi-bootloader.preinst

printf "#!/bin/sh\n" > raspberrypi-kernel-rt.prerm
printf "#!/bin/sh\n" > raspberrypi-kernel-rt.postrm
printf "#!/bin/sh\n" > raspberrypi-kernel-headers-rt.postinst

cat <<EOF >> raspberrypi-kernel-rt.prerm
if [ -f /etc/default/raspberrypi-kernel ]; then
  . /etc/default/raspberrypi-kernel
  INITRD=\${INITRD:-"No"}
  export INITRD
...
cat <<EOF >> raspberrypi-kernel-rt.postrm
if [ -f /etc/default/raspberrypi-kernel ]; then
  . /etc/default/raspberrypi-kernel
  INITRD=\${INITRD:-"No"}
  export INITRD
...
cat <<EOF >> raspberrypi-kernel-headers-rt.postinst
if [ -f /etc/default/raspberrypi-kernel ]; then
  . /etc/default/raspberrypi-kernel
  INITRD=\${INITRD:-"No"}
  export INITRD
